...
3. `pixi run yuimol` で再起動すると各種ツールが使えるようになります。

### 利用可能なツール

| ツール | 概要 |
|--------|------|
| `run_pymol_command` | 任意の PyMOL コマンドを直接実行 |
...
`color_residues_uniprot` は UniProt の位置番号をそのまま指定するだけで、PDB との残基番号のズレを自動補正します。
